...
function BalanceUpdate(props) {
  ...
  if (dataParam) {
    ...
    // Now, dataObject contains your data as a JavaScript object
    console.log("dddddddddddddddddddddddddddddddddd", dataObject);
  }
  ...
  const handleSubmitButton = async () => {
    ...
    await axios
      .post("http://localhost:9000/api/payment", {
      ...
      .then(async () => {
        await axios.put(
          "http://localhost:9000/api/payment/updateAccountBalance",
          {
            userId: user.userId,
    ...
    const fetchAccountBalance = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:9000/api/payment/getCurrentAccountBalance/${userId}`
        );
  ...
  const buySubscription = async () => {
    setIsLoading(true); // Show loading overlay
    console.log("buySubscription", dataParam.plan);
    await axios
      .patch(
        `http://localhost:9000/api/users/updateSubcription/${user.userId}`,
        {
          subscription: "Enterprise package",
          isSubscribed: true,
        }
      )
      .then(() => {
  ...
  return (
    <Styles>
      <div className="card">
        {dataObject != null ? (
          <>
            <h3>Your Total: LKR {dataObject.price}</h3>
          </>
        ) : (
          <>
            <h2>Your balance</h2>
